<?php
  ...
  // $head .= "<script src='https://code.jquery.com/jquery-1.9.1.js'></script>";
  ...
  $table = $head;
    ...
    foreach ($house as $key=>$val) {
      ...
      if( $key == "Spist") {
        ...
        $table .= "</tr>";
          ...
          $table .= "<th style='padding-left: 20px;'>" . $key . "</th><td'><b>" . $val . "</b></td>";
          $table .= "</tr>";
      } else {
          $table .= "<tr style='padding: 0;'>";
    ...
    $folder = generatePdf($table,  $house->Navn) ?? null;
    if($folder) {
      $pdfGenerated++;
    }
  }
...
function generatePdf($html, $name) {
